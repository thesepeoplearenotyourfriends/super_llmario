...
- **`animations`** owns temporal sequences through the order of their `frames`. Current unknown timing is recorded as `frameTiming: "unspecified"`; consumers do not infer playback semantics from adjacent sheet cells, and the integrity test does not freeze that present uncertainty into the future contract.
