...
- **`animations`** owns temporal sequences through the order of their `frames`. Exact frame duration and looping are left unspecified because the source-art review does not establish them; consumers do not infer either property from adjacent sheet cells.
