...
test('rotating block temporary spin disables collision, animates once, and restores idle',()=>{
  ...
  assert.equal(event.triggers[0].type,'temporarySpin');assert(duration>0);assert.equal(surface.disabled,true);assert.equal(first.resource,theme.animations['block.rotating.spin'].frames[0].resource);
