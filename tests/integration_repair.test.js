...
assert.deepEqual(Object.keys(built.recipes).sort(),Object.keys(theme.recipes).sort());
assert.equal(built.audio.defaults.volume,.7);assert.deepEqual(built.audio.sfx.jump,[{note:38}]);
assert(engine.includes('let ACTIVE_CART = DEFAULT_CONTENT.cart;'));
...
assert(reference.platforms.every(p=>p.collisionOnly));
const referenceTerrain=reference.resourceScenery.sprites.filter(s=>s.semanticFamily==='terrain.overground');
assert(referenceTerrain.length>2000);assert(referenceTerrain.every(s=>s.image.startsWith('map.terrain.overground.')));
