'use strict';const assert=require('node:assert/strict'),C=require('../construction/catalog.js');
const terrain={id:'terrain.g',type:'terrain',name:'Ground',components:{topology:{'0000':'solo','0100':'left','0001':'right','0101':'middle',default:'fill'}},ascii:{import:'#',export:'#'}};
...
const catalog={version:1,families:{'terrain.g':terrain,sign:fixed}};assert.deepEqual(C.validate(catalog),[]);assert.deepEqual(C.asciiMaps(catalog),{imports:{'#':'terrain.g'},exports:{'terrain.g':'#'}});assert.equal(C.construct(fixed).length,4);
...
const themeA={constructionCatalog:{version:1,families:{pipe:Object.assign({},pipe,{assets:{ml:'green-mouth'}})}}},themeB={constructionCatalog:{version:1,families:{pipe:Object.assign({},pipe,{assets:{ml:'blue-mouth'}})}}};const stored={semanticFamily:'pipe',semanticRole:'ml',x:8,y:9};assert.equal(C.resolveRole(themeA,stored.semanticRole),'green-mouth');assert.equal(C.resolveRole(themeB,stored.semanticRole),'blue-mouth');assert.deepEqual(stored,{semanticFamily:'pipe',semanticRole:'ml',x:8,y:9});
