...
cp.execFileSync('python3',['scripts/sync_editor_catalog.py','--check']);for(const file of ['archived/editor/editor.html','engine/engine.html']){const html=fs.readFileSync(file,'utf8');assert(!/<script[^>]+src=["'][^"']*construction\/catalog\.js/.test(html));assert(html.includes('BEGIN GENERATED CONSTRUCTION CATALOG'))}
